...
const ZPenghasilan = () => {
  ...
  const GOLD_PRICE = 1000000; // Rp1.000.000 per gram
  const NISAB_GRAM = 85;
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="mb-8">
          <h1 className="text-2xl md:text-4xl font-bold text-emerald-700 flex items-center gap-3">
            <LuTrendingUp className="w-7 h-7 md:w-9 md:h-9" />
            Zakat Penghasilan
          </h1>

          <p className="text-sm md:text-base text-slate-500 mt-3 leading-relaxed">
            Hitung zakat profesi berdasarkan penghasilan bulanan dan bonus
            tambahan.
          </p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-lg border border-slate-100 p-5 md:p-8">
          ...
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex gap-3 mb-6">
            <LuCircleAlert className="w-5 h-5 text-amber-600 flex-shrink-0 mt-1" />
          ...
          {result && (
            ...
            >
              ...
              <div className="space-y-2 text-sm md:text-base">
                <p>
                  <span className="font-semibold">Total Penghasilan:</span> Rp
                  {(
                    (parseFloat(income) || 0) + (parseFloat(otherIncome) || 0)
                  ).toLocaleString()}
                </p>

                <p>
                  <span className="font-semibold">Pendapatan Tahunan:</span> Rp
